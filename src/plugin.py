...
def llmProviderChat(prompt, max_tokens, reasoning_mode):
    ...
    # Reasoning trace (Issue #7), re-ported onto the plugin dispatch: emit an llm_call trace under
    # the current iteration's trace_id. Best-effort — a trace failure never breaks the chat path.
    try:
        try:
            import tracing
